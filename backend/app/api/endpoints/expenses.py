...
def update_expense(*, db: Session = Depends(deps.get_db), expense_id: int, expense_in: ExpenseUpdate):
    return ExpenseService.update(db, expense_id, expense_in)
...
@router.post("/bulk-classify", status_code=200)
...
@router.post("/bulk-update-type", status_code=200)
...
@router.post("/bulk-reclassify", status_code=200)
...
@router.post("/bulk-assign-holiday", status_code=200)
